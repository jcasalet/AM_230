...
def main():
    ...
    while norm(grad_f(x_k)) > epsilon:
        ...
        alpha, fc, gc, new_fval, old_fval, new_slope = scipy.optimize.line_search(f=f, myfprime=grad_f, xk=x_k, pk=p_k, gfk = grad_f(x_k), amax=1, maxiter=100, c1=c1, c2=c2)
        x_kplus1 = x_k + alpha * p_k

        gradf_k = grad_f(x_k)
        gradf_kplus1 = grad_f(x_kplus1)

        restart = abs(gradf_kplus1.transpose().dot(gradf_k)) / (norm(gradf_kplus1) ** 2)
        if restart >= 0.1:
            b_kplus1 = 0
        else:
